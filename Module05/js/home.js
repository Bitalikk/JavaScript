 ...
 */
const getUserByEmail = (arr, email) => arr.filter(el => el.email === email).map(post => post.name);
...
*/
const getAllUniqueSkills = arr =>  {return [...new Set(arr.reduce((acc, el) => el.skills ? acc.concat(el.skills) : acc, []).sort())];};
...
// const getSort = (a, b) => a.friends.length - b.friends.lendth;
