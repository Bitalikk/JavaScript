...
//   const isActive = users.filter(el => el.isActive);
...
// const user = users.find(el => el.id === '001');
// console.log(user);
...
//     let accumulator = args.reduce((acc, el) => acc += el, 0);
...
//       return guests.filter(el => el.isActive);
//   }
//   // Вызовы функции для проверки
...
// function isGuestsActive(guest) {
//     return guest.every(el => el.active);
// }
...
// const abc = scientist.sort(function(a, b) {
...
// console.log(scientist.sort((a, b) => a.year - b.year));

// 5) Відсортувати вчених по кількості прожитих років
// const sortYears = scientist.sort((a, b) => (a.passed - a.year) - (b.passed - b.year));
...
// console.log(scientist.find(el => el.year === scientist.reduce((acc, i) => Math.max(acc, i.year), 0)));

// 8) Знайти рік народження Albert Einstein
// console.log(scientist.find(el => el.first === 'Albert' && el.last === 'Einstein').year);
// 9) Знайти вчених прізвище яких починається на літеру С
// const startC = scientist.filter(el => el.last[0] === 'C');
// console.log(startC);
// 10) Видалити з масива всіх вчених імя яких починається на A
// scientist.filter(el => el.first[0] === 'A').forEach(del => scientist.splice(scientist.indexOf(del), 1));
// console.log(scientist);
...
//  */
// const getUsersByFriend = (arr, name) => arr.filter(el => el.friends.find(fr => fr === name));
...
//   return [...new Set(arr.reduce((acc, el) => acc.concat(el.skills), []))].sort();
// };
